...
// Problem 1: Using .map(), compile a new array of the runners name and email.

let emails = 

runners.map(({ first_name, email}) => ({first_name, email}));

console.log(emails);

// Problem 2: We need more money for the event so filter out everyone that donated under $20 so we can convince them to donate more.
...
// Problem 3: We need to assign everyone a number for the race so please compile a list that has their index and name.

let number = 

runners.forEach(function(currentValue, index){
  console.log('[' + index + '] = ' + currentValue.first_name);
});